...
| **transformer_decoder_top** (full, estimated†) | ~2,611 | ~3,706 | 0 | ~27 | ≤29.2 |
...
*†Full-decoder figures are a sub-module sum (lower bound): systolic_array already includes its 16 PEs, so the processing_element row is not additive. MHA controller logic, feed-forward, decoder FSM, and weight registers are not separately characterised here, so the true LUT/FF totals will be higher. Fmax is the minimum across all characterised sub-modules (layer_norm critical path). Full top-level synthesis exceeded the 16 GB host memory limit in Vivado.*
